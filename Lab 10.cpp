...
public:
    ...
    void load(ifstream& in) {
        ...
        size_t n;
        if (!(in >> n)) {
            cerr << "Error reading inventory size" << endl;
            return;
        }
        ...
        for (size_t i = 0; i < n; ++i) {
            if (!getline(in, tmp)) {
                cerr << "Error reading inventory item" << endl;
                break;
            }
            items.push_back(tmp);
        }
...
public:
    ...
    void load(ifstream& in) {
        in >> ws;
        getline(in, name);
        if (!(in >> hp >> attackPower >> defense >> level >> experience)) {
            cerr << "Error reading character data" << endl;
            return;
        }
        inventory.load(in);
    }
